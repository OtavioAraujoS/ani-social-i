...
export function TopicRow({
  ...
  return (
    <motion.div
      ...
      className={`glass-panel group transition-all duration-100 rounded-xl p-4 md:px-8 md:py-6 grid items-center gap-4 ${
        userIsAdmin ? "grid-cols-12 md:grid-cols-13" : "grid-cols-12"
      }`}
    >
      <div className="col-span-12 md:col-span-6">
        <Link href={`/topicos/${topic.id}`} className="cursor-pointer">
      ...
      <div
        className={`${userIsAdmin ? "col-span-6" : "col-span-12"} md:col-span-1 flex items-center justify-end gap-2 text-slate-500`}
      >
